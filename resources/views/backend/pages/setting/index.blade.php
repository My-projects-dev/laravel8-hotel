                                        ...
                                        <tr>
                                            ...
                                            <td><div class="guest-bx">
                                                @if($setting->parentSetting->image)
                                                    <img src="{{asset('uploads/settings/'.$setting->parentSetting->image)}}" alt="{{$setting->parentSetting->key}}"
                                                         width="100"
                                                         height="100"
                                                         class="me-3 rounded">
                                                @endif </div>
                                            </td>
                                            <td class="job-desk">{{$setting->parentSetting->key}}</td>
                                            <td class="job-desk">{!! $setting->value !!}</td>
                                            <td>
                                                <span
                                                    class="{{($setting->parentSetting->status==1) ? 'text-success' : 'text-danger'}}">{{($setting->parentSetting->status==1) ? 'ACTIVE' : 'INACTIVE'}}</span>
                                            </td>
                                            <td>
                                                <div class="dropdown dropstart">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                           href="{{route('setting.edit',$setting->parentSetting->id)}}">Edit</a>
                                                        <form
                                                            action="{{ route('setting.destroy',$setting->parentSetting->id) }}"
                                                            method="Post">
                                                            @csrf
                                                            @method('DELETE')
